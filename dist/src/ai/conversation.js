...
const tools = [
    {
        ...
        function: {
            ...
            description: "Extract rental requirements from the user's message. Call this whenever the user mentions any requirement.",
            parameters: {
                type: "object",
                properties: {
                    customerName: {
...
export async function chat(openai, messages, currentRequirements) {
    // Build context about what we already know
    const knownInfo = [];
    if (currentRequirements.customerName)
        knownInfo.push(`Customer name: ${currentRequirements.customerName}`);
    if (currentRequirements.customerOrigin)
        knownInfo.push(`Customer origin: ${currentRequirements.customerOrigin}`);
    if (currentRequirements.location)
        knownInfo.push(`Location: ${currentRequirements.location}`);
    if (currentRequirements.bedrooms !== undefined)
        knownInfo.push(`Bedrooms: ${currentRequirements.bedrooms}`);
    if (currentRequirements.maxBudget)
        knownInfo.push(`Budget: ${currentRequirements.maxBudget}`);
    if (currentRequirements.moveInDate)
        knownInfo.push(`Move-in: ${currentRequirements.moveInDate}`);
    if (currentRequirements.extras)
        knownInfo.push(`Extras: ${currentRequirements.extras}`);
    const contextPrompt = knownInfo.length > 0
