...
export function classifyContactMethod(listing) {
    ...
    if (/contact\s*form|inquiry\s*form|message\s*form|send\s*message|contact\s*us|inquiry/i.test(text) ||
...
export async function searchListings(exa, requirements) {
    // Build search query
    ...
    try {
        ...
        for (const item of result.results) {
            ...
            // Extract price
            const priceMatch = textContent.match(/[€$£]?\s*(\d{1,3}(?:[.,]\d{3})*|\d+)(?:\s*(?:€|EUR|USD|\$|£|GBP|\/month|per month|monthly|p\.?m\.?))/i);
            let price = 0;
            let currency = "EUR";
            if (priceMatch) {
...
export function formatListingMessage(listing, index) {
    ...
    const contactInfo = listing.contactMethod && listing.contactMethod !== "form"
        ...
        : listing.contactMethod === "form"
            ...
            : "";
    return (`*${index + 1}\\. ${escapeMarkdown(listing.title)}*\n` +
