...
import { chat } from "../ai/conversation";
import { searchListings, formatListingMessage } from "../search/exa";
export function createBot(token, convex, openai, exa, voiceService, callService, websiteScraper, browserbaseClient, docusignService, paymentService) {
    ...
    bot.command("start", async (ctx) => {
        ...
        await ctx.reply(`👋 Hey ${user.first_name}! I'm InstaRent, your AI rental agent.\n\n` +
            `I'll help you find your perfect rental property.\n\n` +
            `First, what's your name?`);
    });
    bot.command("reset", async (ctx) => {
        ...
        try {
            ...
            await ctx.reply(`🔄 All reset! Let's start fresh.\n\n` +
    ...
    bot.callbackQuery("confirm_appointment", async (ctx) => {
        try {
            ...
            console.warn("Failed to answer callback query for confirm_appointment (possibly too old):", error);
        }
        const user = ctx.from;
        if (!user)
            return;
        try {
            ...
            await ctx.reply("✅ Great! I'll now prepare the rental contract for you. Stand by...");
        }
        catch (error) {
    ...
    bot.callbackQuery("confirm_contract", async (ctx) => {
        try {
            ...
            console.warn("Failed to answer callback query for confirm_contract (possibly too old):", error);
        }
        const user = ctx.from;
        if (!user)
            return;
        try {
            ...
            if (selectedListing) {
                ...
                await ctx.reply(`✅ Contract confirmed!\n\n` +
                ...
                await ctx.reply("✅ Contract confirmed! Now let's handle the payment...");
            }
        }
        catch (error) {
...
Be precise and only return information you can clearly read.`
            ...
            // Confirm to user
            await ctx.reply(`✅ Passport information extracted!\n\n` +
            ...
            if (docusignService) {
                try {
                    ...
                    const envelope = await docusignService.createContractAndGetSigningUrl(contractDetails, user.id.toString());
    ...
    bot.callbackQuery("confirm_payment", async (ctx) => {
        try {
            ...
            console.warn("Failed to answer callback query for confirm_payment (possibly too old):", error);
        }
        const user = ctx.from;
        if (!user)
            return;
        try {
            ...
            await ctx.reply(`🎉 Congratulations! Your rental process is complete.\n\n` +
    ...
    bot.callbackQuery(/^select_(\d+)$/, async (ctx) => {
        ...
        try {
            ...
            await ctx.reply(`✅ Great choice\\! You selected:\n\n` +
                ...
                `I'll now contact the lister on your behalf\\. Stand by for updates\\! 📞`, { parse_mode: "MarkdownV2" });
            // Send voice confirmation if available
            if (voiceService) {
                try {
                    const voiceBuffer = await voiceService.generateUpdateMessage(`Great choice! You selected ${selectedListing.title}. ` +
            ...
            if (contactMethod === "form") {
                ...
                if (browserbaseClient) {
                    try {
                        ...
                        await browserbaseClient.act(`Fill out the contact form with the following information:
              - Name: Mike Lee
              - Role: Property Agent
              ...
              Then submit the form.`);
                    ...
                    await ctx.reply("📝 Browserbase is not configured. Cannot fill out contact form. " +
                ...
                if (callService && selectedListing.contactPhone) {
                    try {
                        ...
                        await ctx.reply(`📞 Call initiated to the property lister as Mike Lee!\n` +
                            `Call ID: \`${callResult.callSid}\`\n\n` +
                            `I'll update you when I hear back.`, { parse_mode: "Markdown" });
                    }
                    catch (callError) {
                        console.error("Call error:", callError);
                        await ctx.reply("📱 I couldn't reach the lister by phone right now. " +
                            "I'll try again later or reach out via email if available.");
                    }
                }
                else if (!callService) {
                    await ctx.reply("📱 Twilio is not configured. Cannot make phone calls. " +
                        "Please configure Twilio credentials to enable phone calls.");
                }
                else if (!selectedListing.contactPhone) {
                    await ctx.reply("📱 No phone number available for this listing. " +
                ...
                if (callService && selectedListing.contactPhone) {
                    try {
                        ...
                        await ctx.reply(`📞 Call initiated to the property lister as Mike Lee (LINE contact method - using phone as fallback)!\n` +
                            `Call ID: \`${callResult.callSid}\`\n\n` +
                            `I'll update you when I hear back.`, { parse_mode: "Markdown" });
                    }
                    catch (callError) {
                    ...
                    await ctx.reply("💬 LINE contact method detected, but phone fallback is not available. " +
                ...
                if (callService && selectedListing.contactPhone) {
                    try {
                        ...
                        console.error("Call error:", callError);
                        await ctx.reply("📱 I couldn't reach the lister right now. " +
                            "I'll try again later.");
                    }
                }
                else {
                    await ctx.reply("❓ Unknown contact method. Please check the listing details.");
                }
            }
        }
        catch (error) {
    ...
    bot.on("message:text", async (ctx) => {
        ...
        try {
            ...
            const currentRequirements = conversation?.requirements || {};
            ...
            if (conversation?.status === "contracting" && !currentRequirements.passportData) {
...
Return ONLY valid JSON. If any required field (fullName, passportNumber, dateOfBirth) is missing, set "error" to true.`
                    ...
                    if (extractedData.error || !extractedData.fullName || !extractedData.passportNumber || !extractedData.dateOfBirth) {
                        await ctx.reply(`⚠️ I couldn't extract all the required information from your message.\n\n` +
                            `Please provide:\n` +
                            `• Your full name\n` +
                            `• Passport number\n` +
                            `• Date of birth\n` +
                    ...
                    await ctx.reply(`✅ Thank you! I've received your information:\n\n` +
                        `Name: ${passportData.fullName}\n` +
                        `Passport #: ${passportData.passportNumber}\n` +
                    ...
                    if (docusignService) {
                        try {
                            ...
                            console.error("❌ Error creating DocuSign contract:", docusignError);
                            await ctx.reply(`❌ I had trouble creating the contract. Please try again later or contact support.\n\n` +
                    ...
                    await ctx.reply(`⚠️ I had trouble processing your information. Please try again with:\n\n` +
                        `Your full name\n` +
                        `Passport number\n` +
                        `Date of birth\n` +
                        `Nationality`);
            ...
            const response = await chat(openai, messages.map((m) => ({ role: m.role, content: m.content })), currentRequirements);
                ...
                if (exa || websiteScraper) {
                    ...
                    try {
                        ...
                        if (finalListings.length === 0) {
                            await ctx.reply("😕 I couldn't find any listings matching your criteria. " +
                        ...
                        await ctx.reply(`🏠 Found ${finalListings.length} listings for you:\n\n` +
                        ...
                        await ctx.reply("😕 Had trouble searching. Please try again or use /reset to start over.");
                    }
                }
                else {
                    await ctx.reply("⚠️ Search is not configured yet. Please set up EXA_API_KEY or SMITHERY_BROWSERBASE_URL for website scraping.");
                }
            }
            else {
            ...
            try {
                await ctx.reply("Sorry, I ran into an issue. Please try again or use /reset to start over.");
            }
            catch (replyError) {
