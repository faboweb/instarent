...
export async function createWebsiteScraper(config) {
    let browser = null;
    ...
    async function scrapePropertySites(location) {
        ...
        for (let i = 0; i < PROPERTY_SITES.length; i++) {
            ...
            try {
                ...
                const rawListings = (await client.extract(`Extract all property rental listings visible on this page. For each listing, get:
          ...
          Make sure to capture ALL images for each listing, not just one.`, extractionSchema));
...
export async function scrapeChiangMaiPropertySites(config) {
