 ...
 */
export function constructSmitheryUrl(apiKey, profile) {
    const url = new URL("https://server.smithery.ai/@browserbasehq/mcp-browserbase/mcp");
    url.searchParams.set("api_key", apiKey);
    url.searchParams.set("profile", profile);
    return url.toString();
}
export async function createBrowserbaseClient(config) {
    ...
    const client = new Client({ name: "instarent-scraper", version: "1.0.0" }, { capabilities: {} });
    try {
        ...
        if (error?.code === 400 || error?.event?.code === 400 || error?.status === 400) {
            ...
            throw new Error(`Failed to connect to ${urlType} server: Invalid URL or credentials (400). ` +
    ...
    return {
        ...
        async extract(instruction, schema) {
            const args = { sessionId, instruction };
            if (schema) {
                args.schema = schema;
            }
