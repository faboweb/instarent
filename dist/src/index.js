...
import { createVoiceService } from "./voice/elevenlabs";
import { createOutboundCallService } from "./voice/twilio";
import { createCalendlyService } from "./scheduling/calendly";
import { createPaymentService } from "./payments/stripe";
import { createDocuSignService } from "./contracts/docusign";
...
async function checkForScheduledAppointment(convex, conversationId, appointment, calendlyService, bot) {
    try {
        ...
        for (const event of scheduledEvents) {
            ...
            if (eventStartTime > appointmentCreatedAt || nameMatch) {
                ...
                // Notify user
                await bot.api.sendMessage(user.telegramId, `✅ Great! I see you've scheduled your viewing for:\n\n` +
...
const requiredEnvVars = ["TELEGRAM_BOT_TOKEN", "OPENAI_API_KEY", "CONVEX_URL"];
...
async function main() {
    ...
    if (process.env.ELEVENLABS_API_KEY) {
        voiceService = createVoiceService(process.env.ELEVENLABS_API_KEY, process.env.ELEVENLABS_VOICE_ID);
...
async function resumeStuckConversations(convex, openai, exa, bot, callService, calendlyService, docusignService, paymentService) {
    try {
        ...
        for (const conversation of activeConversations) {
            ...
            // Handle "scheduling" stuck conversations
                ...
                if (appointment && calendlyService) {
                    ...
                    const wasScheduled = await checkForScheduledAppointment(convex, conversation._id, appointment, calendlyService, bot);
                    if (!wasScheduled) {
...
async function performSearchAndNotify(convex, exa, conversationId, requirements, bot) {
    try {
        ...
        try {
            await bot.api.sendMessage(user.telegramId, `🏠 Found ${listings.length} listings for you:\n\n` +
...
async function handleContactingState(convex, conversationId, bot, callService, calendlyService, openai) {
    try {
        ...
        if (latestCall.status === "pending" && !latestCall.twilioCallSid && callService) {
            ...
            try {
                ...
                // Notify user
                try {
                    await bot.api.sendMessage(user.telegramId, `📞 Call initiated to the property lister as Mike Lee!\n` +
                ...
                if (isPermissionError) {
                    try {
                        ...
                        if (conversation) {
                            ...
                            if (user) {
                                await bot.api.sendMessage(user.telegramId, `❌ I encountered a configuration issue making the call. ` +
        ...
        // Otherwise, determine from Twilio status
            ...
            if (latestCall.transcript && latestCall.transcript.length > 0) {
                ...
                if (transcriptLower.includes("call back") ||
                    ...
                    const hasAvailability = availabilityKeywords.some(keyword => transcriptLower.includes(keyword));
                    if (hasAvailability) {
        ...
        if (callState === "done") {
            ...
            if (!user)
                return;
            try {
                await bot.api.sendMessage(user.telegramId, `📞 The property lister asked for a callback. ` +
                ...
                if (conversation) {
                    ...
                    if (user) {
                        try {
                            ...
                            await bot.api.sendMessage(user.telegramId, `📞 The call was missed. I'll try again in ${hoursUntilRetry} hour${hoursUntilRetry > 1 ? 's' : ''}.`);
                        }
                        catch (error) {
                            console.error(`  ❌ Failed to notify user:`, error);
                        }
            ...
            if (nextRetryTime && new Date(nextRetryTime).getTime() <= now) {
                if (retryAttempt >= backoffIntervals.length) {
                    ...
                    if (conversation) {
                        ...
                        if (user) {
                            try {
                                await bot.api.sendMessage(user.telegramId, `❌ I had trouble reaching the property lister after multiple attempts. ` +
                ...
                if (selectedListing && selectedListing.contactPhone && callService) {
                    ...
                    try {
                        ...
                        await bot.api.sendMessage(user.telegramId, `📞 Retrying the call now (attempt ${retryAttempt + 2})...`);
                    }
                    catch (error) {
            ...
            if (!nextRetryTime && retryAttempt < backoffIntervals.length) {
                ...
                if (conversation) {
                    ...
                    if (user) {
                        try {
                            await bot.api.sendMessage(user.telegramId, `I didn't reach the property right now. I will try again later today.`);
                        }
                        catch (error) {
                            console.error(`  ❌ Failed to notify user:`, error);
                        }
            ...
            if (nextRetryTime && new Date(nextRetryTime).getTime() <= now) {
                ...
                if (conversation) {
                    ...
                    if (user) {
                        try {
                            await bot.api.sendMessage(user.telegramId, `❌ I had trouble reaching the property lister. ` +
                                `Would you like to select a different listing?`);
                        }
                        catch (error) {
                            console.error(`  ❌ Failed to notify user:`, error);
...
async function extractAvailabilityFromTranscript(openai, transcript) {
    try {
...
async function handleSchedulingState(convex, conversationId, bot, calendlyService, openai) {
    try {
        ...
        if (!selectedListing) {
            ...
            await bot.api.sendMessage(user.telegramId, `⚠️ No property selected. Please select a property first.`);
            return;
        }
        ...
        if (calendlyService) {
            try {
                ...
                const schedulingLink = await calendlyService.createPropertyViewingLink(selectedListing.title, clientName, undefined, // No email available
                ...
                const alreadyScheduled = await checkForScheduledAppointment(convex, conversationId, appointment, calendlyService, bot);
                if (!alreadyScheduled) {
                    ...
                    const keyboard = new InlineKeyboard().text("✅ I've scheduled my appointment", "confirm_appointment");
...
async function handleContractingState(convex, conversationId, bot, docusignService) {
    try {
        ...
        if (!contract) {
            ...
            if (docusignService) {
                try {
                    const envelope = await docusignService.createContractAndGetSigningUrl(contractDetails, user.telegramId.toString());
                    ...
                    const keyboard = new InlineKeyboard().text("✅ I've signed the contract", "confirm_contract");
                    await bot.api.sendMessage(user.telegramId, message, {
                        reply_markup: keyboard,
                    });
            ...
            if (contract.docusignEnvelopeId && docusignService) {
                try {
                    ...
                    if (status === "completed") {
                        ...
                        await bot.api.sendMessage(user.telegramId, `✅ Contract signed successfully! Now let's handle the payment.`);
                    }
                }
                catch (error) {
...
async function handlePaymentState(convex, conversationId, bot, paymentService) {
    try {
        ...
        if (payment) {
            ...
            await bot.api.sendMessage(user.telegramId, `🎉 Congratulations! Your rental process is complete.\n\n` +
                `Property: ${selectedListing.title}\n` +
        ...
        if (!payment) {
            ...
            if (paymentService) {
                try {
                    const invoice = await paymentService.createRentalPaymentInvoice(tenantEmail, tenantName, selectedListing.title, monthlyRent, deposit, currency);
                    ...
                    const keyboard = new InlineKeyboard().text("✅ I've completed the payment", "confirm_payment");
                    await bot.api.sendMessage(user.telegramId, message, {
                        reply_markup: keyboard,
                    });
            ...
            if (payment.stripeInvoiceId && paymentService) {
                try {
                    ...
                    if (invoiceStatus.status === "paid") {
                        ...
                        await bot.api.sendMessage(user.telegramId, `🎉 Payment received! Congratulations!\n\n` +
                        ...
                        await bot.api.sendMessage(user.telegramId, `❌ There was an issue with the payment. Please try again or contact support.`);
                    }
                }
                catch (error) {
...
async function scrapeOnStartup(scraper, convex) {
    try {
